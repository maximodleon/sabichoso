...
const evaluateMoviePage = () => {
  ...
  let dayTime = []
  for (let div of divs) {
    const divParent = div.querySelector('div[class="small-12 medium-12 large-12 columns"]')
    const movieTheater = divParent.querySelector('h6').innerText
    const times = divParent.querySelectorAll('div[class="row collapse"]>div[class="small-12 medium-11 large-11 columns"]>ul>li')
    for (let time of times) {
      const ps = time.querySelectorAll('p')
      const day = { hours: [] }
      for (let p of ps) {
        if (['Jue', 'Vie', 'Sab', 'Dom', 'Lun', 'Mar', 'Mie'].includes(p.innerText)) {
